import {ConfirmationModalService, ConfirmationOptions, ModalOptions, ModalSize} from '@healthcatalyst/cashmere';
import {Component} from '@angular/core';
import { FormControl } from '@angular/forms';

/**
 * @title Confirmation Modal Overview
 */
@Component({
    selector: 'hc-modal-confirmation-overview-example',
    templateUrl: 'modal-confirmation-overview-example.component.html',
    styleUrls: ['modal-confirmation-overview-example.component.scss']
})
export class ModalConfirmationOverviewExampleComponent {
    constructor(private confirmService: ConfirmationModalService) {}
    readonly message = new FormControl('', {nonNullable: true});
    readonly messageHTML = new FormControl(`<p> Are you sure you to purchase this <strong>Didgeridoo</strong>?</p><p>It is a <em>really cool</em> wind instrument, played with vibrating lips to produce a continuous drone while using a special breathing technique called circular breathing.</p>`, {nonNullable: true});
    ...
    open(): void {
        ...
        const contentOptions: ConfirmationOptions = {
            message: this.message.value,
            messageHTML: this.messageHTML.value,
        ...
        this.confirmService.confirm(contentOptions, modalOptions).subscribe(confirmed => {
            if(confirmed) {
