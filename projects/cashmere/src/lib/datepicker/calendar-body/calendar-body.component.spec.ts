...
describe('CalendarBodyComponent', () => {
    ...
    describe('standard calendar body', () => {
        ...
        it('should set aria-selected correctly', () => {
            ...
            expect(deselectedCells.length).toBe(cellEls.length - 1, 'Expected remaining cells to be marked as deselected.');
        });
        ...
        it('cell should be selected on click', () => {
            const todayElement = calendarBodyNativeElement.querySelector('.hc-calendar-body-today') as HTMLElement;
            todayElement.click();
            fixture.detectChanges();

            expect(todayElement.classList).toContain('hc-calendar-body-selected', 'today should be selected');
        });
